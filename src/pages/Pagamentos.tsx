...
import {
    CheckCircle, AlertTriangle, Clock, Copy, X,
...
const configGestor = {
    ...
    pixName: 'Ubus Mobilidade Ltda',
    notificationDaysBefore: 3,
}

const parcelas: Parcela[] = [
    { id: 1, mes: 'Janeiro/2026', valor: 150, vencimento: '10/01/2026', status: 'pago', canAdvance: false },
    { id: 2, mes: 'Fevereiro/2026', valor: 150, vencimento: '10/02/2026', status: 'pago', canAdvance: false },
    { id: 3, mes: 'Março/2026', valor: 150, vencimento: '10/03/2026', status: 'atrasado', canAdvance: false },
    { id: 4, mes: 'Abril/2026', valor: 150, vencimento: '10/04/2026', status: 'pendente', canAdvance: false },
    { id: 5, mes: 'Maio/2026', valor: 150, vencimento: '10/05/2026', status: 'futuro', canAdvance: true },
    { id: 6, mes: 'Junho/2026', valor: 150, vencimento: '10/06/2026', status: 'futuro', canAdvance: true },
]
...
export default function Pagamentos() {
    ...
    const handleCopyPix = () => {
        navigator.clipboard.writeText(configGestor.pixKey)
        setPixCopied(true)
            ...
            <div className="flex-1 px-6 pb-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm mb-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className={`px-3 py-1.5 rounded-full text-xs font-bold ${atrasadas.length > 0
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-400">
                        <Bell size={12} />
                        <span>Aviso {configGestor.notificationDaysBefore} dias antes</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                    {visibleParcelas.map((parcela) => {
                        ...
                        const canAdvancePay = parcela.status === 'futuro' && parcela.canAdvance && configGestor.allowAdvancePayment
                            ...
                            <div key={parcela.id} className={`rounded-2xl border ${cfg.border} ${cfg.bg} p-4 transition-all`}>
                                ...
                                {(canPay || canAdvancePay) && (
                                    <button
                                        ...
                                        className={`w-full mt-3 py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-[0.98] ${parcela.status === 'atrasado'
                ...
                {parcelas.length > 4 && (
                    <button
                        ...
                        className="w-full mt-3 flex items-center justify-center gap-1.5 py-3 text-sm text-primary font-medium"
                    >
                        {showAll ? (
                            <><ChevronUp size={16} /> Ver menos</>
                        ) : (
                            <><ChevronDown size={16} /> Ver todas ({parcelas.length})</>
                        )}
                    </button>
                )}
            </div>
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-t-3xl p-6 pb-10 animate-slide-up">
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex items-center gap-3 bg-slate-100 rounded-xl p-4">
                                <p className="flex-1 text-sm font-mono font-medium text-slate-900 select-all">{configGestor.pixKey}</p>
                                <button
                                    onClick={handleCopyPix}
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${pixCopied
                                            ? 'bg-emerald-100 text-emerald-600'
                        ...
                        <div className="bg-blue-50 rounded-xl p-4 mb-6 flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-blue-700/80 mt-0.5">{configGestor.pixName}</p>
                            </div>
                        </div>

                        <p className="text-[11px] text-slate-400 text-center leading-relaxed">
                            Após o pagamento, envie o comprovante para o gestor para confirmação.
                            A baixa pode levar até 24h úteis.
                        </p>
                    </div>
                </div>
            )}
