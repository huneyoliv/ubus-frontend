...
type UserType = 'aluno' | 'motorista' | 'gestor'
...
export default function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('http://129.80.98.178/api/auth/login', {
                method: 'POST',
            ...
            const userType: UserType = data.user?.type ?? data.type ?? 'aluno'
            ...
            setAuth(data.token, {
                ...
                type: userType,
            }, data.user?.pagamentos ?? false, data.user?.lider ?? false)
        ...
        <div className="w-full max-w-md mx-auto min-h-screen relative overflow-hidden bg-white flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-6 py-2 w-full">
                <label className="flex flex-col w-full">
                    <p className="text-slate-900 text-sm font-medium leading-normal pb-2">CPF</p>
